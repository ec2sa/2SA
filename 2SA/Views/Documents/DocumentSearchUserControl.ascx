...
<!--
    ...
    function setSortIcon(){
        ...
        if(type=="CreateDate"){
            $("#hCreateDate").append(" " + icon);
        }
    }
    
    function sortDocument(sortType){
        var actualSortType = $("#SortType").val();
        var actualSortDirection = $("#SortDirection").val();
        
        if(actualSortType == sortType){
            if(actualSortDirection == "Ascending"){
    ...
    function changePage(pageNo){
        //$.getAllQueryStrings()["p"].value;
        var form = $("form:last");
        var action = form.attr("action");
        var newAction = action.substring(0,action.indexOf("p=")) + "p="+pageNo;
