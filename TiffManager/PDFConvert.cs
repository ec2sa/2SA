    ...
    public class PDFConvert
    {
        ...
        private bool Convert(string inputFile, string outputFile,bool throwException,string options)
        {
            ...
            if (string.IsNullOrEmpty(inputFile))
            {
                ...
                {
                    System.Diagnostics.Trace.WriteLine("The inputfile is missing");
                    return false;
                }
            }
            if (!System.IO.File.Exists(inputFile))
            {
                ...
                {
                    System.Diagnostics.Trace.WriteLine(string.Format("The file :'{0}' doesn't exist", inputFile));
                    return false;
                }
            }
            if (string.IsNullOrEmpty(_sDeviceFormat))
            {
                ...
                {
                    System.Diagnostics.Trace.WriteLine("You didn't provide a device for the conversion");
                    return false;
                }
            }
            ...
            try
            {
                ...
                if (intReturn < 0)
                {
                    ...
                    {
                        System.Diagnostics.Trace.WriteLine("I can't create a new istance of Ghostscript please verify no other istance are running!");
                        return false;
                    }
                }
            }
            ...
            {//in this case the dll we r using isn't the dll we expect
                ...
                else
                {
                    ...
                    System.Diagnostics.Trace.WriteLine("The gsdll32.dll wasn't found in default dlls search path" +
                        "or is not in correct version (doesn't expose the required methods). Please download " +
                        "the version 8.64 from the original website");
                    return false;
                }
